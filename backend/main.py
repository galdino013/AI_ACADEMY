...
from fastapi import FastAPI, HTTPException, Response, Depends, status, Query # ✅ NOVO: Importa Query para o token
...
from itsdangerous import URLSafeTimedSerializer, SignatureExpired, BadTimeSignature # ✅ NOVO: Importa exceções ItsDangerous

try:
    from google import genai as google_genai_sdk
except Exception as e:
    print(f"⚠️ Aviso: SDK 'google-genai' não pôde ser importado: {e}")
    google_genai_sdk = None

try:
    import openai
except Exception as e:
    print(f"⚠️ Aviso: SDK 'openai' não pôde ser importado: {e}")
    openai = None
...
GEMINI_MODELS = ["gemini-2.5-flash"] # Usando um modelo Flash como padrão

# ✅ NOVO: Configurações de E-mail e Token de Confirmação
EMAIL_CONFIRMATION_SECRET = os.getenv("EMAIL_CONFIRMATION_SECRET")
FRONTEND_URL = os.getenv("FRONTEND_URL", "http://localhost:5173") # URL do seu frontend

# --- Inicialização dos Clientes de IA ---
genai_client = None
if google_genai_sdk and GEMINI_API_KEY:
    try:
        google_genai_sdk.configure(api_key=GEMINI_API_KEY) # Configurar a chave da API
        # Para modelos generativos de texto, normalmente se usa o GenerativeModel com 'gemini-pro' ou 'gemini-flash'
        # ou, se o GEMINI_MODELS tiver um nome de modelo que o SDK reconhece diretamente:
        genai_client = google_genai_sdk.GenerativeModel(GEMINI_MODELS[0]) # Usando o primeiro modelo da lista
        logger.info("Google GenAI SDK inicializado com sucesso.")
    except Exception as e:
        logger.warning(f"⚠️ Aviso: Falha ao configurar ou instanciar Google GenAI SDK: {e}")
        genai_client = None
        google_genai_sdk = None
else:
    logger.warning("⚠️ Aviso: Google GenAI SDK ou API Key não configurados. Funcionalidades Gemini podem estar indisponíveis.")

if openai and OPENAI_API_KEY:
    try:
        openai.api_key = OPENAI_API_KEY # OpenAI SDK v0.28.1+ configura assim
        logger.info("OpenAI SDK inicializado com sucesso.")
    except Exception as e:
        logger.warning(f"⚠️ Aviso: Falha ao configurar OpenAI SDK: {e}")
        openai = None
else:
    logger.warning("⚠️ Aviso: OpenAI SDK ou API Key não configurados. Funcionalidades OpenAI podem estar indisponíveis.")

# --- Configuração do Serializador para Tokens de Confirmação ---
serializer = None
...
logger.setLevel(logging.INFO) # Definir nível de log para o logger
...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"], # Considere restringir isso para o seu frontend em produção
...
async def get_current_user(token: str = Depends(oauth2_scheme), db: Session = Depends(get_db)) -> models.User:
    ...
    # ✅ NOVO: Verifica se o usuário está ativo mesmo para acessar rotas protegidas
...
def _run_sync_genai_generate(model_name: str, prompt: str) -> str:
    ...
    try:
        # Usando o genai_client instanciado globalmente
        response = genai_client.generate_content(contents=prompt)
        return response.text.strip()
    except Exception as e:
        logger.error(f"Erro detalhado do _run_sync_genai_generate: {e}")
        raise
...
async def gerar_resumo_gemini_async(pergunta: str, contexto: str) -> str:
    ...
    for modelo in GEMINI_MODELS:
        try:
            ...
            logger.warning("Falha com Gemini model %s (Async, SDK Novo): %s", modelo, e)
            continue
...
async def gerar_resumo_openai_async(pergunta: str, contexto: str) -> str:
    ...
    try:
        ...
        logger.warning("Falha na chamada 'client.chat.completions.create' (Async): %s", e)
        raise
...
async def gerar_resumo_por_fallback(pergunta: str, resultados: List[Dict[str, Any]]) -> str:
    ...
    logger.info("Ambas IAs falharam, usando fallback local.")
...
async def perguntar(
    ...
    if not q:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Pergunta vazia.")
    ...
    if not resultados:
        try:
            if google_genai_sdk is not None:
                ...
                try:
                    # Usando o genai_client globalmente instanciado
                    resp_opt = genai_client.generate_content(prompt_opt)
                    termo_otimizado = resp_opt.text.strip().strip('\"')
