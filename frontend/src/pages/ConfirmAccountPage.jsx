...
const API_BASE_URL = 'https://aiacademy2025.netlify.app/';
const ConfirmAccountPage = () => {
  ...
  useEffect(() => {
    ...
    const confirmAccount = async () => {
      try {
        const response = await fetch(`${API_BASE_URL}/users/confirm-account?token=${token}`, {
          method: 'GET',
          headers: {
      ...
      <div className="auth-form">
        ...
        {loading ? (
          ...
          <>
            ...
            {!error && (
              <p>Você já pode <Link to="/login">fazer login</Link>!</p>
            )}
            {error && (
              <p><Link to="/register">Tentar registrar novamente</Link> ou <Link to="/login">fazer login</Link>.</p>
            )}
          </>
        )}
